...
public partial class site_listdata : System.Web.UI.Page
{
    ...
    private string GetPagedContent()
    {
        ...
        {
            var q = from u in ctx.SITEs
                    where u.DELETED == 0 select u;

            if (!string.IsNullOrEmpty(qtype) && !string.IsNullOrEmpty(query))
            {
                q = q.Where(u => u.DELETED ==0 && u.NAME.ToLower().Contains(query.ToLower()));
            }
            ...
            {
                var fgRow = new FlexigridRow {id = result.ID, cell = new List<string> {result.NAME}};
