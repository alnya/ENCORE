<%@ Page Title="" Language="C#" MasterPageFile="~/master/twocolumn.master" AutoEventWireup="true" CodeFile="default.aspx.cs" Inherits="report_default" %>
    ...
    <asp:ListView ID="rptReports" runat="server" DataSourceID="edsReports">
        <ItemTemplate>
            <div class="panel dataentry reportpanel clearfix">
                <div class="reportstatus <%#Eval("RequestStatus") %>"><%#Eval("RequestStatus") %></div>
                <h2><a href='detail.aspx?ID=<%#Eval("ID") %>'><%#Eval("Name") %></a></h2>
                <div class="reportactions">
                    <asp:HyperLink ID="lnkViewData" CssClass="action data" runat="server" Visible='<%# (Eval("RequestStatus").ToString()==com.Encore.Web.RequestStatus.Complete.ToString()) ? true : false %>' NavigateURL='<%# "viewreport.aspx?ID=" + Eval("ID") %>' Text="View Data" />
                    <asp:HyperLink ID="lnkViewGraph" CssClass="action graph" runat="server" Visible='<%# (Eval("RequestStatus").ToString()==com.Encore.Web.RequestStatus.Complete.ToString()) ? true : false %>' NavigateURL='<%# "viewgraph.aspx?ID=" + Eval("ID") %>' Text="View Graph" />
                </div>
            </div>
        </ItemTemplate>
    ...
    <asp:panel runat="server" ID="pnlNewRecord" CssClass="panel noheader" DefaultButton="btnNewAccount">
        ...
        <script language="javascript">
            $(".reportstatus").click(function () {
                ...
                if (lnk.hasClass("New")) {
                    lnk.removeClass("New");
                    lnk.addClass("Pending");
                    Encore.showToast('Success', 'Loading Data for this report');
                }
            });
    </script>
